...
def validate_translator(main_page, source_lang, target_lang):
    ...
    if (translator_tool == tr("Microsoft Translator") and 
        ...
        print("Error: Microsoft Translator API key not found")
        return False
        
    if (translator_tool == tr("Yandex") and 
        not credentials.get(tr("Yandex"), {}).get("api_key")):
        print("Error: Yandex API key not found")
        return False
    ...
    if ('Claude' in translator_tool and 
        ...
        print("Error: Anthropic Claude API key not found")
        return False
    
    if ('Grok' in translator_tool and 
        not credentials.get(tr('Grok AI'), {}).get('api_key')):
        print("Error: Grok AI API key not found")
        return False
    
    # Check service-specific incompatibilities
    if translator_tool == tr('DeepL'):
        if target_lang == main_page.tr('Traditional Chinese'):
            print("Error: DeepL does not support Traditional Chinese")
            return False
        if target_lang == main_page.tr('Thai'):
            print("Error: DeepL does not support Thai")
            return False
        if target_lang == main_page.tr('Vietnamese'):
            print("Error: DeepL does not support Vietnamese")
            return False
    ...
    if  translator_tool == tr('Google Translate'):
            if (source_lang == main_page.tr('Brazilian Portuguese') or 
                ...
                print("Error: Google Translate does not support Brazilian Portuguese")
                return False
